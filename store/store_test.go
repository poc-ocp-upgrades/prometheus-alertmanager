...
func TestSetGet(t *testing.T) {
	...
	a := NewAlerts(d)
	a.Run(ctx)
	alert := &types.Alert{UpdatedAt: time.Now()}
...
func TestGC(t *testing.T) {
	...
	newAlert := func(key string, start, end time.Duration) *types.Alert {
		return &types.Alert{Alert: model.Alert{Labels: model.LabelSet{model.LabelName(key): "b"}, StartsAt: now.Add(start * time.Minute), EndsAt: now.Add(end * time.Minute)}}
	}
	active := []*types.Alert{newAlert("b", 10, 20), newAlert("c", -10, 10)}
	resolved := []*types.Alert{newAlert("a", -10, -5), newAlert("d", -10, -1)}
