...
func testJoinLeave(t *testing.T) {
	...
	require.NotNil(t, p)
	err = p.Join(DefaultReconnectInterval, DefaultReconnectTimeout)
	require.NoError(t, err)
	...
	p2, err := Create(logger, prometheus.NewRegistry(), "0.0.0.0:0", "", []string{p.Self().Address()}, true, DefaultPushPullInterval, DefaultGossipInterval, DefaultTcpTimeout, DefaultProbeTimeout, DefaultProbeInterval)
	require.NoError(t, err)
	require.NotNil(t, p2)
	err = p2.Join(DefaultReconnectInterval, DefaultReconnectTimeout)
	require.NoError(t, err)
...
func testReconnect(t *testing.T) {
	...
	go p.Settle(context.Background(), 0*time.Second)
	p.WaitReady()
	p2, err := Create(logger, prometheus.NewRegistry(), "0.0.0.0:0", "", []string{}, true, DefaultPushPullInterval, DefaultGossipInterval, DefaultTcpTimeout, DefaultProbeTimeout, DefaultProbeInterval)
	require.NoError(t, err)
	require.NotNil(t, p2)
	err = p2.Join(DefaultReconnectInterval, DefaultReconnectTimeout)
	require.NoError(t, err)
...
func testRemoveFailedPeers(t *testing.T) {
	...
	n := p.Self()
	now := time.Now()
	p1 := peer{status: StatusFailed, leaveTime: now, Node: n}
	p2 := peer{status: StatusFailed, leaveTime: now.Add(-time.Hour), Node: n}
	p3 := peer{status: StatusFailed, leaveTime: now.Add(30 * -time.Minute), Node: n}
