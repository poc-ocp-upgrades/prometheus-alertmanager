...
func (api *API) getStatusHandler(params general_ops.GetStatusParams) middleware.Responder {
	...
	if api.peer != nil {
		...
		for _, n := range api.peer.Peers() {
			address := n.Address()
			peers = append(peers, &open_api_models.PeerStatus{Name: &n.Name, Address: &address})
		}
		resp.Cluster = &open_api_models.ClusterStatus{Name: &name, Status: &status, Peers: peers}
	}
...
func (api *API) getAlertsHandler(params alert_ops.GetAlertsParams) middleware.Responder {
	...
	if params.Receiver != nil {
		...
		if err != nil {
			return alert_ops.NewGetAlertsBadRequest().WithPayload(fmt.Sprintf("failed to parse receiver param: %v", err.Error()))
		}
	}
	...
	for a := range alerts.Next() {
		...
		alert := open_api_models.GettableAlert{Alert: open_api_models.Alert{GeneratorURL: strfmt.URI(a.GeneratorURL), Labels: modelLabelSetToAPILabelSet(a.Labels)}, Annotations: modelLabelSetToAPILabelSet(a.Annotations), StartsAt: &startsAt, UpdatedAt: &updatedAt, EndsAt: &endsAt, Fingerprint: &fingerprint, Receivers: receivers, Status: &open_api_models.AlertStatus{State: &state, SilencedBy: status.SilencedBy, InhibitedBy: status.InhibitedBy}}
...
func gettableSilenceFromProto(s *silencepb.Silence) (open_api_models.GettableSilence, error) {
	...
	sil := open_api_models.GettableSilence{Silence: open_api_models.Silence{StartsAt: &start, EndsAt: &end, Comment: &s.Comment, CreatedBy: &s.CreatedBy}, ID: &s.Id, UpdatedAt: &updated, Status: &open_api_models.SilenceStatus{State: &state}}
	for _, m := range s.Matchers {
		matcher := &open_api_models.Matcher{Name: &m.Name, Value: &m.Pattern}
		switch m.Type {
		...
		case silencepb.Matcher_REGEXP:
			...
			matcher.IsRegex = &t
		default:
			return sil, fmt.Errorf("unknown matcher type for matcher '%v' in silence '%v'", m.Name, s.Id)
		}
...
func (api *API) postSilencesHandler(params silence_ops.PostSilencesParams) middleware.Responder {
	...
	if err != nil {
		...
		return silence_ops.NewPostSilencesBadRequest().WithPayload(fmt.Sprintf("failed to convert API silence to internal silence: %v", err.Error()))
	}
	...
	return silence_ops.NewPostSilencesOK().WithPayload(&silence_ops.PostSilencesOKBody{SilenceID: sid})
}
