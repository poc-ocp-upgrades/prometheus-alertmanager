...
func TestAPI(t *testing.T) {
	...
	now := time.Now()
	statusData := &ServerStatus{ConfigYAML: "{}", ConfigJSON: &config.Config{}, VersionInfo: map[string]string{"version": "v1"}, Uptime: now, ClusterStatus: &ClusterStatus{Peers: []PeerStatus{}}}
	doStatus := func() (interface{}, error) {
		api := httpStatusAPI{client: client}
		return api.Get(context.Background())
	}
	alertOne := Alert{StartsAt: now, EndsAt: now.Add(time.Duration(5 * time.Minute)), Labels: LabelSet{"label1": "test1"}, Annotations: LabelSet{"annotation1": "some text"}}
	alerts := []*ExtendedAlert{{Alert: alertOne, Fingerprint: "1c93eec3511dc156", Status: types.AlertStatus{State: types.AlertStateActive}}}
	...
	silOne := &types.Silence{ID: "abc", Matchers: []*types.Matcher{{Name: "label1", Value: "test1", IsRegex: false}}, StartsAt: now, EndsAt: now.Add(time.Duration(2 * time.Hour)), UpdatedAt: now, CreatedBy: "alice", Comment: "some comment", Status: types.SilenceStatus{State: "active"}}
