...
func TestMatcherMatch(t *testing.T) {
	...
	tests := []struct {
		matcher		Matcher
		expected	bool
	}{{matcher: Matcher{Name: "label", Value: "value"}, expected: true}, {matcher: Matcher{Name: "label", Value: "val"}, expected: false}, {matcher: Matcher{Name: "label", Value: "val.*", IsRegex: true}, expected: true}, {matcher: Matcher{Name: "label", Value: "diffval.*", IsRegex: true}, expected: false}, {matcher: Matcher{Name: "difflabel", Value: "value"}, expected: false}}
...
func TestMatchersMatch(t *testing.T) {
	...
	tests := []struct {
		matchers	Matchers
		expected	bool
	}{{matchers: Matchers{m1, m2}, expected: true}, {matchers: Matchers{m1, m3}, expected: false}}
...
func TestMatchersEqual(t *testing.T) {
	...
	tests := []struct {
		...
		matchers2	Matchers
		expected	bool
	}{{matchers1: Matchers{m1, m2}, matchers2: Matchers{m1, m2}, expected: true}, {matchers1: Matchers{m1, m3}, matchers2: Matchers{m1, m2}, expected: false}}
	for _, test := range tests {
