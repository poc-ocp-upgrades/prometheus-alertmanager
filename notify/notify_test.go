...
func TestDedupStageNeedsUpdate(t *testing.T) {
	...
	cases := []struct {
		...
		resolve		bool
		res		bool
	}{{entry: nil, firingAlerts: alertHashSet(2, 3, 4), res: true}, {entry: nil, resolvedAlerts: alertHashSet(2, 3, 4), res: false}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2, 3}}, firingAlerts: alertHashSet(2, 3, 4), res: true}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2, 3}, Timestamp: time.Time{}}, firingAlerts: alertHashSet(1, 2, 3), res: true}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2, 3}, Timestamp: now.Add(-9 * time.Minute)}, repeat: 10 * time.Minute, firingAlerts: alertHashSet(1, 2, 3), res: false}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2, 3}, Timestamp: now.Add(-11 * time.Minute)}, repeat: 10 * time.Minute, firingAlerts: alertHashSet(1, 2, 3), res: true}, {entry: &nflogpb.Entry{ResolvedAlerts: []uint64{1, 2, 3}, Timestamp: now.Add(-11 * time.Minute)}, repeat: 10 * time.Minute, resolvedAlerts: alertHashSet(3, 4, 5), resolve: true, res: false}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2}, ResolvedAlerts: []uint64{3}, Timestamp: now.Add(-9 * time.Minute)}, repeat: 10 * time.Minute, firingAlerts: alertHashSet(1), resolvedAlerts: alertHashSet(2, 3), resolve: false, res: false}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2}, ResolvedAlerts: []uint64{3}, Timestamp: now.Add(-9 * time.Minute)}, repeat: 10 * time.Minute, firingAlerts: alertHashSet(1), resolvedAlerts: alertHashSet(2, 3), resolve: true, res: true}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2}, ResolvedAlerts: []uint64{3}, Timestamp: now.Add(-9 * time.Minute)}, repeat: 10 * time.Minute, firingAlerts: alertHashSet(), resolvedAlerts: alertHashSet(1, 2, 3), resolve: false, res: true}, {entry: &nflogpb.Entry{FiringAlerts: []uint64{1, 2}, ResolvedAlerts: []uint64{3}, Timestamp: now.Add(-9 * time.Minute)}, repeat: 10 * time.Minute, firingAlerts: alertHashSet(), resolvedAlerts: alertHashSet(1, 2, 3), resolve: true, res: true}}
	for i, c := range cases {
		t.Log("case", i)
		s := &DedupStage{now: func() time.Time {
			return now
		}, conf: notifierConfigFunc(func() bool {
			return c.resolve
		})}
...
func TestDedupStage(t *testing.T) {
	...
	i := 0
	now := utcNow()
	s := &DedupStage{hash: func(a *types.Alert) uint64 {
		res := uint64(i)
		i++
		return res
	}, now: func() time.Time {
	...
	alerts := []*types.Alert{{}, {}, {}}
	s.nflog = &testNflog{qerr: errors.New("bad things")}
	ctx, _, err = s.Exec(ctx, log.NewNopLogger(), alerts...)
	require.EqualError(t, err, "bad things")
	s.nflog = &testNflog{qerr: nflog.ErrNotFound}
	...
	s.nflog = &testNflog{qerr: nil, qres: []*nflogpb.Entry{{FiringAlerts: []uint64{0, 1, 2}}, {FiringAlerts: []uint64{1, 2, 3}}}}
	ctx, _, err = s.Exec(ctx, log.NewNopLogger(), alerts...)
	require.Contains(t, err.Error(), "result size")
	i = 0
	s.nflog = &testNflog{qerr: nflog.ErrNotFound, qres: []*nflogpb.Entry{{FiringAlerts: []uint64{0, 1, 2}, Timestamp: now}}}
	ctx, res, err = s.Exec(ctx, log.NewNopLogger(), alerts...)
	require.NoError(t, err)
	require.Nil(t, res, "unexpected alerts returned")
	i = 0
	s.nflog = &testNflog{qerr: nil, qres: []*nflogpb.Entry{{FiringAlerts: []uint64{1, 2, 3, 4}, Timestamp: now}}}
...
func TestMultiStage(t *testing.T) {
	...
	stage := MultiStage{StageFunc(func(ctx context.Context, l log.Logger, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
...
func TestRoutingStage(t *testing.T) {
	...
	stage := RoutingStage{"name": StageFunc(func(ctx context.Context, l log.Logger, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
		if !reflect.DeepEqual(alerts, alerts1) {
...
func TestRetryStageWithError(t *testing.T) {
	...
	alerts := []*types.Alert{&types.Alert{Alert: model.Alert{EndsAt: time.Now().Add(time.Hour)}}}
	ctx := context.Background()
...
func TestRetryStageNoResolved(t *testing.T) {
	...
	r := RetryStage{integration: i}
	alerts := []*types.Alert{&types.Alert{Alert: model.Alert{EndsAt: time.Now().Add(-time.Hour)}}, &types.Alert{Alert: model.Alert{EndsAt: time.Now().Add(time.Hour)}}}
	ctx := context.Background()
	resctx, res, err := r.Exec(ctx, log.NewNopLogger(), alerts...)
...
func TestSetNotifiesStage(t *testing.T) {
	...
	defer _logClusterCodePath()
	tnflog := &testNflog{}
	s := &SetNotifiesStage{recv: &nflogpb.Receiver{GroupName: "test"}, nflog: tnflog}
	alerts := []*types.Alert{{}, {}, {}}
	ctx := context.Background()
...
func TestSilenceStage(t *testing.T) {
	...
	if _, err := silences.Set(&silencepb.Silence{EndsAt: utcNow().Add(time.Hour), Matchers: []*silencepb.Matcher{{Name: "mute", Pattern: "me"}}}); err != nil {
		t.Fatal(err)
	}
	marker := types.NewMarker()
	silencer := NewSilenceStage(silences, marker)
	in := []model.LabelSet{{}, {"test": "set"}, {"mute": "me"}, {"foo": "bar", "test": "set"}, {"foo": "bar", "mute": "me"}, {}, {"not": "muted"}}
	out := []model.LabelSet{{}, {"test": "set"}, {"foo": "bar", "test": "set"}, {}, {"not": "muted"}}
	var inAlerts []*types.Alert
	for _, lset := range in {
		inAlerts = append(inAlerts, &types.Alert{Alert: model.Alert{Labels: lset}})
	}
