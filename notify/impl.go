...
func BuildReceiverIntegrations(nc *config.Receiver, tmpl *template.Template, logger log.Logger) []Integration {
	...
	var (
		...
		add		= func(name string, i int, n Notifier, nc notifierConfig) {
			integrations = append(integrations, Integration{notifier: n, conf: nc, name: name, idx: i})
		}
	)
...
func (w *Webhook) Notify(ctx context.Context, alerts ...*types.Alert) (bool, error) {
	...
	msg := &WebhookMessage{Version: "4", Data: data, GroupKey: groupKey}
...
func (n *Email) Notify(ctx context.Context, as ...*types.Alert) (bool, error) {
	...
	if len(n.conf.Text) > 0 {
		w, err := multipartWriter.CreatePart(textproto.MIMEHeader{"Content-Transfer-Encoding": {"quoted-printable"}, "Content-Type": {"text/plain; charset=UTF-8"}})
		if err != nil {
	...
	if len(n.conf.HTML) > 0 {
		w, err := multipartWriter.CreatePart(textproto.MIMEHeader{"Content-Transfer-Encoding": {"quoted-printable"}, "Content-Type": {"text/html; charset=UTF-8"}})
		if err != nil {
...
func (n *PagerDuty) notifyV2(ctx context.Context, c *http.Client, eventType, key string, data *template.Data, details map[string]string, as ...*types.Alert) (bool, error) {
	...
	msg := &pagerDutyMessage{Client: tmpl(n.conf.Client), ClientURL: tmpl(n.conf.ClientURL), RoutingKey: tmpl(string(n.conf.RoutingKey)), EventAction: eventType, DedupKey: hashKey(key), Images: make([]pagerDutyImage, len(n.conf.Images)), Links: make([]pagerDutyLink, len(n.conf.Links)), Payload: &pagerDutyPayload{Summary: tmpl(n.conf.Description), Source: tmpl(n.conf.Client), Severity: tmpl(n.conf.Severity), CustomDetails: details, Class: tmpl(n.conf.Class), Component: tmpl(n.conf.Component), Group: tmpl(n.conf.Group)}}
...
func (n *Slack) Notify(ctx context.Context, as ...*types.Alert) (bool, error) {
	...
	if numFields > 0 {
		...
		for index, field := range n.conf.Fields {
			...
			fields[index] = config.SlackField{Title: tmplText(field.Title), Value: tmplText(field.Value), Short: &short}
		}
	...
	if numActions > 0 {
		...
		for index, action := range n.conf.Actions {
			slackAction := config.SlackAction{Type: tmplText(action.Type), Text: tmplText(action.Text), URL: tmplText(action.URL), Style: tmplText(action.Style), Name: tmplText(action.Name), Value: tmplText(action.Value)}
			if action.ConfirmField != nil {
				slackAction.ConfirmField = &config.SlackConfirmationField{Title: tmplText(action.ConfirmField.Title), Text: tmplText(action.ConfirmField.Text), OkText: tmplText(action.ConfirmField.OkText), DismissText: tmplText(action.ConfirmField.DismissText)}
			}
			actions[index] = slackAction
		}
		attachment.Actions = actions
	}
	req := &slackReq{Channel: tmplText(n.conf.Channel), Username: tmplText(n.conf.Username), IconEmoji: tmplText(n.conf.IconEmoji), IconURL: tmplText(n.conf.IconURL), LinkNames: n.conf.LinkNames, Attachments: []slackAttachment{*attachment}}
	if err != nil {
		return false, err
	}
	var buf bytes.Buffer
...
func (n *Hipchat) Notify(ctx context.Context, as ...*types.Alert) (bool, error) {
	...
	req := &hipchatReq{From: tmplText(n.conf.From), Notify: n.conf.Notify, Message: msg, MessageFormat: n.conf.MessageFormat, Color: tmplText(n.conf.Color)}
	if err != nil {
		return false, err
	}
	var buf bytes.Buffer
...
func (n *Wechat) Notify(ctx context.Context, as ...*types.Alert) (bool, error) {
	...
	msg := &weChatMessage{Text: weChatMessageContent{Content: tmpl(n.conf.Message)}, ToUser: tmpl(n.conf.ToUser), ToParty: tmpl(n.conf.ToParty), Totag: tmpl(n.conf.ToTag), AgentID: tmpl(n.conf.AgentID), Type: "text", Safe: "0"}
	if err != nil {
		return false, fmt.Errorf("templating error: %s", err)
	}
	var buf bytes.Buffer
...
func (n *OpsGenie) createRequest(ctx context.Context, as ...*types.Alert) (*http.Request, bool, error) {
	...
	default:
		...
		msg = &opsGenieCreateMessage{Alias: alias, Message: message, Description: tmpl(n.conf.Description), Details: details, Source: tmpl(n.conf.Source), Teams: teams, Tags: tags, Note: tmpl(n.conf.Note), Priority: tmpl(n.conf.Priority)}
	}
...
func (n *VictorOps) createVictorOpsPayload(ctx context.Context, as ...*types.Alert) (*bytes.Buffer, error) {
	...
	msg := map[string]string{"message_type": messageType, "entity_id": hashKey(key), "entity_display_name": tmpl(n.conf.EntityDisplayName), "state_message": stateMessage, "monitoring_tool": tmpl(n.conf.MonitoringTool)}
	if err != nil {
